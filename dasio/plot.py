...
def plot_timeline(db, ax=None, by_month=None):
    """Gantt-style coverage timeline for a `DASdb`.

    Green where data exists, red for the acquisition gaps between.
    ...
    # Only this plotter needs pandas; `imshow` and `wiggle` are numpy-only and
    # this module is imported for them far more often.
    import pandas as pd
    ...
    ax.set_title(
        f'{db.format}  |  {db.n_files:,} files  |  '
        f'{db.n_segments} segments  |  '
